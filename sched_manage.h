...
class TASK
{
    private:
        ...
        time_t remind_time;
        static int counter;  // 静态计数器，用于分配唯一的 ID

    public:
        TASK();
        TASK(string name,int b,int c, time_t a, time_t d);
...
class GUEST
{
    ...
    public:
        ...
        void Save_to_File();
        void addTask(string name,int priority, int category, string start_time,  string reminder_time);
        bool printTask_by_reminder_time();
        void printTask_by_id();
        void printTask_by_start_time();
        void printTask_by_type();
        void printTask_by_priority();
        bool deleteTask(int id);
        bool changeTask(int id);
        bool checktask();
};
